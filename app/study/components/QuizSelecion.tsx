...
import { Maximize2, ArrowLeft, Save } from 'lucide-react';
...
export function Quiz({ focusMode = false, setFocusMode, subjectId }: QuizProps) {
  ...
  const handleSaveQuiz = async (payload: AIQuizPayload) => {
    ...
    try {
      ...
      await createQuizMutation.mutateAsync(createPayload);

      toast.success('Quiz salvo na sua coleção!');

      setQuestions([]);
      setShowResults(false);
      setUserAnswers([]);
    } catch {
      toast.error('Falha ao salvar quiz. Tente novamente.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="flex gap-2 justify-end sticky top-0 bg-background z-10 p-2 border-b">
        {focusMode ? (
            ...
            {subjectId && (
              <div className="flex gap-2 justify-center">
                <Button
                  onClick={() =>
                    handleSaveQuiz({
                      tema: questions[0]?.tema || 'Quiz',
                      dificuldade: questions[0]?.dificuldade || 'Médio',
                      quantidade: questions.length,
                    })
                  }
                  disabled={isSaving || createQuizMutation.isPending}
                  className="gap-2"
                >
                  <Save className="w-4 h-4" />
                  {isSaving || createQuizMutation.isPending ? 'Salvando...' : 'Adicionar à Coleção'}
                </Button>
              </div>
            )}
          </div>
